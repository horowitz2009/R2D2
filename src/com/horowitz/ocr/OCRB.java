...
public class OCRB {
  ...
  public OCRB(String prefix, ImageComparator comparator) throws IOException {
    ...
    Map<Integer, BufferedImage> _digitsMap = new HashMap<>();
    _digits = new BufferedImage[11];
    ...
    try {
      String filename = prefix + "slash.bmp";
      URL url = ImageManager.getImageURL(filename);
      BufferedImage image = null;
      if (url != null)
        image = ImageIO.read(url);
  ...
  public String scanImage(BufferedImage image) {
    ...
    while (subimage.getWidth() >= wmin) {
      ...
      if (found.size() == 1) {
        ...
        if (subimage.getWidth() - _digits[m].getWidth() <= 0) {
          // it's over
          break;
        }
        subimage = subimage.getSubimage(0 + _digits[m].getWidth(), 0, subimage.getWidth() - _digits[m].getWidth(),
            subimage.getHeight());
   ...
   * BufferedImage subimage; // cut north boolean lineClean = true; int yStart =
   * 0; for (int y = 0; y < image.getHeight(); y++) {
   * 
   * for (int x = 0; x < image.getWidth(); x++) { int diff =
   * compareTwoColors(image.getRGB(x, y), foreground.getRGB()); if (diff <=
   * 1100) { // found one, line not clean lineClean = false; break; } } if
   * (!lineClean) { yStart = y; // enough break; } } subimage =
   * image.getSubimage(0, yStart, image.getWidth(), image.getHeight() - yStart);
   * writeImage(subimage, 3);
   * 
   * // cut south lineClean = true; yStart = subimage.getHeight() - 1; for (int
   * y = subimage.getHeight() - 1; y >= 0; y--) {
   * 
   * for (int x = 0; x < subimage.getWidth(); x++) { int diff =
   * compareTwoColors(subimage.getRGB(x, y), foreground.getRGB()); if (diff <=
   * 1100) { // found one, line not clean lineClean = false; break; } } if
   * (!lineClean) { yStart = y; // enough break; } } subimage =
   * subimage.getSubimage(0, 0, subimage.getWidth(), yStart + 1);
   * writeImage(subimage, 4); // cut west boolean colClean = true; int xStart =
   * 0; for (int xx = 0; xx < subimage.getWidth(); xx++) {
   * 
   * for (int y = 0; y < subimage.getHeight(); y++) { int diff =
   * compareTwoColors(subimage.getRGB(xx, y), foreground.getRGB()); if (diff <=
   * 1100) { // found one, line not clean colClean = false; break; } } if
   * (!colClean) { xStart = xx; if (xStart > 0) xStart--; // enough break; } }
   * subimage = subimage.getSubimage(xStart, 0, subimage.getWidth() - xStart,
   * subimage.getHeight()); writeImage(subimage, 5); // cut east colClean =
   * true; xStart = subimage.getWidth() - 1; for (int xx = subimage.getWidth() -
   * 1; xx >= 0; xx--) {
   * 
   * for (int y = 0; y < subimage.getHeight(); y++) { int diff =
   * compareTwoColors(subimage.getRGB(xx, y), foreground.getRGB()); if (diff <=
   * 1100) { // found one, line not clean colClean = false; break; } } if
   * (!colClean) { xStart = xx; if (xStart < subimage.getWidth() - 1) xStart++;
   * // enough break; } } subimage = subimage.getSubimage(0, 0, xStart + 1,
   * subimage.getHeight()); writeImage(subimage, 6); return subimage; }
   */
